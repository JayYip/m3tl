...
import nbformat
import random
from nbdev.imports import Config, parallel
...
def before_test(nb):
    ...
    cells = [(i, c, e) for i, (c, e) in enumerate(
        zip(nb['cells'], exports)) if c['cell_type'] == 'code']
    for i, c, e in cells:
        if e:
            # if it's exported to the library, don't run as test
            c['cell_type'] = 'exclude'
            # but we might still need to run import statements
            for line in split_flags_and_code(c):
                if 'import' in line:
                    imports += f'{line}\n'
            continue
...
def nbdev_test_nbs(fname=None, flags=None, n_workers=None, verbose=True, timing=False):
    ...
    if flags is not None:
        flags = flags.split(' ')
    if fname is None:
        # files = [f for f in Config().nbs_path.glob(
        #     '*.ipynb') if not f.name.startswith('_')]
        files = [f for f in glob.glob(os.path.join(
            Config().nbs_path, '*.ipynb')) if not f.startswith('_')]
    else:
        files = glob.glob(fname)
    files = [Path(f).absolute() for f in sorted(files)]
    if len(files) == 1 and n_workers is None:
        n_workers = 0
    # make sure we are inside the notebook folder of the project
    os.chdir(Config().nbs_path)
    results = parallel(_test_one, files, flags=flags,
